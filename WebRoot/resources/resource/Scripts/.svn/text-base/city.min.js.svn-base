...
$(document).ready(function(){
    var getData=$("#Area i")
    var data=new Object();
        data.province=$("#Province i").text
        data.city=$("#City i").text
        data.area=$("#Area i").text;
        var jsonMsg=JSON.stringify(data);
        localStorage.setItem(data.name,jsonMsg);//通过查找姓名来查找相关的数据  
        getData.onclick = function()//获取数据
            {
                var province = localStorage.getItem("data");
                console.log(data)
                if(province)
                {
