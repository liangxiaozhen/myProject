...
$("#uname").click(
		function() {
			...
			$('#unameMSG').addClass("prompt_1 remind_1").html(
					"<i></i>" + minlength + "~" + maxlength
							+ "位的字符，可包含数字、英文字母、“_”、“-”");
		});
// 用户名输入框失去焦点
$("#uname").blur(
		function() {
			...
			if (unameVal == '') {// 用户名为空
				unameFlag = false;
				$('#unameMSG').css("display", "");
				$('#unameMSG').removeClass().addClass("prompt_1 error_1").html(
						"<i></i>用户名不能为空");
			} else if (unameVal.length < minlength
					|| unameVal.length > maxlength) {// 长度有误：5~15位字符
				unameFlag = false;
				$('#unameMSG').css("display", "");
				$('#unameMSG').removeClass().addClass("prompt_1 error_1").html(
						"<i></i>用户名长度为" + minlength + "~" + maxlength + "位字符");
			} else if (!unameVal.match("^[a-zA-Z0-9-_]+$") && ischinese == 2) {// 参数有误：中文、数字、英文字母、“_”、“-”
				// ^[\u4e00-\u9fa5_a-zA-Z0-9-]+$
				unameFlag = false;
				$('#unameMSG').css("display", "");
				$('#unameMSG').removeClass().addClass("prompt_1 error_1").html(
						"<i></i>用户名只能包含英文字母、数字、“-”、“_”");
			}
			// else if (namerule === "b") {
			// if (unameVal.match("^[0-9]")) {// 用户名不能以数字开头
			// unameFlag = false;
			// $('#unameMSG').css("display", "");
			// $('#unameMSG').removeClass().addClass("prompt_1 error_1")
			// .html("<i></i>用户名不能以数字开头");
			// }
			// }
			else if (unameVal.match("^[-]")) {
				unameFlag = false;
				$('#unameMSG').css("display", "");
				$('#unameMSG').removeClass().addClass("prompt_1 error_1").html(
						"<i></i>用户名不能以“-”开头");
				...
				unameFlag = true;
				$('#unameMSG').css("display", "");
				$("#unameMSG").removeClass().addClass("prompt_1 correct_1")
						.html("<i></i>");
			}
		});
...
$("#pwd").click(
		function() {
			pwdFlag = false;
			$('#pwdMSG').css("display", "");
			$('#pwdMSG').addClass("prompt_1 remind_1").html(
					"<i></i>8～20位字符，必须包含数字和字母，可包含符号");
		});
// 密码输入框失去焦点
$("#pwd").blur(
		function() {
			...
			if (pwdVal == '') {// 密码输入为空
				...
				pwdFlag = true;
				$('#pwdMSG').css("display", "");
				$("#pwdMSG").removeClass().addClass("prompt_1 correct_1").html(
						"<i></i>");
			}
		});
...
$("#mobile").click(
		function() {
			...
			$('#mobileMSG').addClass("prompt_1 remind_1").html(
					"<i></i>请填写您的真实手机号码，以便下一步手机验证");
		});
// 手机号输入框失去焦点
$("#mobile")
		.blur(
				function() {
					...
					if (mobileVal == '') {// 手机号为空
						mobileFlag = false;
						$('#mobileMSG').css("display", "");
						$('#mobileMSG').removeClass().addClass(
								"prompt_1 error_1").html("<i></i>手机号码不能为空");
					} else if (mobileVal.length != 11) {// 密码长度为6~20位字符
						repwdFlag = false;
						$('#mobileMSG').css("display", "");
						$('#mobileMSG').removeClass().addClass(
								"prompt_1 error_1").html("<i></i>请输入11位手机号码");
						// }else
						// if(!mobileVal.match("^((13[0-9])|(14[5|7])|(15[0|1|2|3|5|6|7|8|9])|(17[3|6|7|8])|18[0-9])\\d{8}|(170\\d{8})$")){//手机号码格式错误
						// 新增171、172、174、175、179号段注册
					} else if (namerule === "b" && mobileVal == usernameVal) {
						mobileFlag = false;
						$('#unameMSG').css("display", "");
						$('#unameMSG').removeClass().addClass(
								"prompt_1 error_1").html("<i></i>用户名不能和手机号相同");
					} else if (!mobileVal
							.match("^((13[0-9])|(14[5|7])|(15[0|1|2|3|5|6|7|8|9])|(17[0-9])|18[0-9])\\d{8}$")) {
						mobileFlag = false;
						$('#mobileMSG').css("display", "");
						$('#mobileMSG').removeClass().addClass(
...
$("#imgCode").blur(
		function() {
			...
			if (imgCodeVal == '') {// 输入为空
				imgCodeFlag = false;
				$('#imgCodeMSG').css("display", "");
				$('#imgCodeMSG').removeClass().addClass("prompt_1 error_1")
						.html("<i></i>验证码不能为空");
				alert(imgCodeVal);
			} else {// 以上验证通过
				$.ajax({
					...
					success : function(msg) {
						if (msg == 1) {
							...
							imgCodeFlag = false;
							$('#imgCodeMSG').css("display", "");
							$("#imgCodeMSG").removeClass().addClass(
									"prompt_1 error_1").html("<i></i>验证码错误");
							// 刷新验证码
							$("#refush").click();
						}
					}
...
function registerBtnClick() {
	...
	if (imgcodeVal == "success") {
		...
		$('#imgCodeMSG').css("display", "");
		$('#imgCodeMSG').removeClass().addClass("prompt_1 error_1").html(
				"<i></i>请拖动滑块完成验证！");
	}
	if (checkInput2()) {
		...
		// 保存信息
		$.post("registerFirstSteps.action", {
			mobile : $('#mobile').val(),
			uname : $('#uname').val(),
			pwd : $('#pwd').val(),
			repwd : $('#repwd').val(),
			...
			if (data['errCode'] == '0') {
				...
				if (data['mobileErrMsg'] != null) {
					unameFlag = false;
					$('#mobileMSG').css("display", "");
					$('#mobileMSG').removeClass().addClass("prompt_1 error_1")
							.html("<i></i>" + data['mobileErrMsg']);
				}
				// 图片验证码验证结果
				if (data['imgCodeErrMsg'] != null) {
					...
					$('#imgCodeMSG').css("display", "");
					$('#imgCodeMSG').removeClass().addClass("prompt_1 error_1")
							.html("<i></i>" + data['imgCodeErrMsg']);
				}
				/*
				 * 协议验证结果 if (data['acpErrMsg'] != null) { acpFlag = false;
				 * $('#acpMSG').css("display", "");
				 * $("#acpMSG").removeClass().addClass("prompt_1 remind_1")
				 * .html("<i></i>" + data['acpErrMsg']); }
				 */
			}
...
function checkInput2() {
	if (unameFlag && pwdFlag && repwdFlag && mobileFlag && imgCodeFlag) {
		return true;
	} else {
		return false;
	}
...
function acpCheck() {
	if ($("#acp").attr("checked") == "checked") {
		...
		$('#acpMSG').css("display", "");
		$("#acpMSG").removeClass().addClass("prompt_1 remind_1").html(
				"<i></i>请选中你我贷用户协议");
	}
}
...
/*
 * $("#uname,#pwd,#repwd,#mobile,#imgCode").change(function() { var data = {
 * "uname" : "注册表单：用户名", "pwd" : "注册表单：密码", "repwd" : "注册表单：确认密码", "mobile" :
 * "注册表单：手机号码", "imgCode" : "注册表单：验证码" }; var key = $(this).attr("name"); //
 * 发送数据 s.prop9 = data[key]; // 注册表单对应的表单项 npo.t(); });
 */
$(function() {
	// 推荐码
	...
	$("#userAgreement").click(function() {
		$(".plusBank1").css("width", "480px");
		$(".plusBank1").css("height", "400px");
		showCon_1();
		$(".plusBank1").show();
	});
});