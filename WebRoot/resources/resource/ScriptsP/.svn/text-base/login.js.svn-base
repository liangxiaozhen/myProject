...
$(function () {
	...
	if(getCookie("nwdUser")){
		//$('#username').removeClass("user_bg1");
		//$('#username').removeClass("user_bg2");
	...
	function validateDate(){
		...
		$.post("/index.do?method=isValidateCode", param, function(date){
            ...
            if (date['validate'] == "1") {
            	...
            	$('#errorMsg').html("<i class='ico_all size15 img_icon s_tan mr_5'></i>手机号或邮箱异常，请使用用户名登录");
    ...
    $('#phoneCodeBut').click(function () {
    	...
    	if(username==""){
    		$('#errorMsg').css("display","block");
        	$('#errorMsg').html("<i class='ico_all size15 img_icon s_cuo mr_5'></i>用户名不能为空");
        	return false;
    	}
    	if(imgCode==""){
    		$('#errorMsg').css("display","block");
        	$('#errorMsg').html("<i class='ico_all size15 img_icon s_cuo mr_5'></i>图形验证码不能为空");
    	...
    	$('#errorMsg').html();
        $.post("/msg/loginSendTextMessage.do", { username: username,imgCode:imgCode },
            function (result) {
        		if(result=='succ'){
        			...
        			//showMsg(textMsg);
        	        $('#errorMsg').css("display","block");
                	$('#errorMsg').html("<i class='ico_all size15 img_icon s_tan mr_5'></i>验证码已发送至您的手机，有效期30分钟，请查收！");
        			...
        			$('#errorMsg').css("display","block");
                	$('#errorMsg').html("<i class='ico_all size15 img_icon s_tan mr_5'></i>图片验证码超时,请刷新！");
        			...
        			$("#phoneCodeBut").removeClass("yzm2").addClass("yzm").attr('disabled', false);
        			$('#errorMsg').css("display","block");
                	$('#errorMsg').html("<i class='ico_all size15 img_icon s_tan mr_5'></i>请输入正确的图片验证码");
        			...
        			$("#phoneCodeBut").removeClass("yzm2").addClass("yzm").attr('disabled', false);
        			$('#errorMsg').css("display","block");
                	$('#errorMsg').html("<i class='ico_all size15 img_icon s_tan mr_5'></i>" + result);
  ...
  //发送语音短信验证码   
    $('#yybtn').click(function () {
    	...
    	$('#errorMsg').html();
        $.post("/msg/loginSendYuyinMessage.do?interval=60", { username: username,imgCode:imgCode },
        		function (result) {
		    		if(result=='succ'){
		    			...
		    			$('#errorMsg').css("display","block");
	                	$('#errorMsg').html("<i class='ico_all size15 img_icon s_tan mr_5'></i>验证码已发送，你我贷将通过400-7910-888进行语音播报！");
    ...
    $('#yybtn2').click(function () {
    	...
    	$('#errorMsg').html();
        $.post("/msg/loginSendYuyinMessage.do?interval=120", { username: username },
        		function (result) {
		    		if(result=='succ'){
		    			/** 20140505 lizhi 修改语音验证码设置为不可以点击样式, 并修改提示信息显示 **/
    ...
    $('#userLoginBtn').click(function () {
        ...
        var _geetestCode = $('#geetestcode').val();
        var _phoneCode = $('#validatePhoneCode').val();
        var _codeType = $('#codeType').val();
        $.currType = $("input[name='curr_type']").val();
        if (_account.length < 1) {
        	$('#errorMsg').css("display","block");
        	$('#errorMsg').html("<i class='ico_all size15 img_icon s_tan mr_5'></i>用户名不能为空");
            ...
            $('#errorMsg').css("display","block");
        	$('#errorMsg').html("<i class='ico_all size15 img_icon s_tan mr_5'></i>密码不能为空");
        ...
        if(_codeType==1){
        	if(_phoneCode.length < 1){
        		$('#errorMsg').css("display","block");
            	$('#errorMsg').html("<i class='ico_all size15 img_icon s_tan mr_5'></i>请输入正确的短信校验码！");
        ...
        if(_codeType==2){
        	if ($.currType == 'geetest') {
        		if(_geetestCode=="success" ){
        			$("#geetestcode").attr('name','imgCode');
        			$("#geetestcode").val("success");
            	}else{
                	$('#errorMsg').html("<i class='ico_all size15 img_icon s_tan mr_5' ></i>请拖动滑块完成验证！").show();
                	return false;
            	}
        	} else {
        		if(_imgCode.length < 1){
            		$('#errorMsg').css("display","block");
                	$('#errorMsg').html("<i class='ico_all size15 img_icon s_tan mr_5' ></i>请输入正确的图片校验码！");
        ...
        $('#errorMsg').css("display","block");
    	$('#errorMsg').html("<i class='ico_all size15 img_icon s_tan mr_5'></i>正在提交验证，请稍候...");
...
//                	}
                	...
                	$('#errorMsg').css("display","block");
                	 $('#errorMsg').html("<i class='ico_all size15 img_icon s_cuo mr_5'></i>"+_data['error']);
                	...
                	if(_codeType==1){
                		if(_data['phoneMsgState'] == 0){
	                		$('#errorMsg').css("display","block");
	                    	$('#errorMsg').html("<i class='ico_all size15 img_icon s_cuo mr_5'></i>用户名或密码错误次数过多，稍后请重新获取验证码！");
	                    	$('#userLoginBtn').attr('disabled', false);
	                    	$('#phoneCode').removeClass("red");
	                    	//去除其他框错误样式
	                    	$('#username').removeClass("red");
                	...
                	if ($.currType == 'geetest') {
                		$('#userLoginBtn').attr('disabled', false);
                    	$('#errorMsg').html("<i class='ico_all size15 img_icon s_cuo mr_5'></i>"+_data['error']).show();
                    	$("#geetestcode").val("");
                    	$(".gt_refresh_tips").click();
                	}else{
                		$('#errorMsg').css("display","block");
    ...
    function getPhoneCode(){
    	...
    	if(flag == true){
    		...
    		$.post("/msg/phoneMsgLoginSendTextMessage.do",{phone: $('#mobile').val(),imgCode: $("#imgquickcode").val()}, function (result){
    	    	if(result == 'succ'){
    ...
    function mobileCh(){
    	...
    	if(mobileVal == ''){//手机号为空
    		...
    		quickLoginHighlight(["mobile"]);
    		return false;
    	}else if(!mobileVal.match("^((13[0-9])|(14[5|7])|(15[0|1|2|3|5|6|7|8|9])|(17[3|6|7|8])|18[0-9])\\d{8}|(170[0|5|9|8|7]\\d{7})$")){//手机号码格式错误
    		//mobileFlag = false;
    ...
    $("#quickLogin").click(function(){
    	...
    	if(imgCodeText == ''){
    		showMsg("quickLoginError","图片验证码不能为空。");
    	...
    	$('#quickLoginErrorMsg').html("<i class='ico_all size15 img_icon s_tan mr_5'></i>正在提交验证，请稍候...");
    ...
    function showErr(id,msg){		
    	$('#'+id+"Msg").html("<i class='ico_all size15 img_icon s_cuo mr_5'></i>"+msg);
    ...
    function showMsg(id,msg){
    	$("#"+id+"Msg").html("<i class='ico_all size15 img_icon s_tan mr_5'></i>"+msg);
    	$("#"+id+"Msg").css("display","block");
    }
    /***
     * 清除验证提示
