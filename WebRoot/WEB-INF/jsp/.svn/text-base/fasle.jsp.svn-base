...
<script type="text/javascript" src="<%=basePath%>haihang/Scripts/jquery-1.11.3.min.js"></script>
<script type="text/javascript" src="<%=basePath%>haihang/Scripts/visitorapi-1.2.1-min.js"></script>
<script type="text/javascript" src="<%=basePath%>haihang/Scripts/appmeasurement-1.2.1-min.js"></script>
<link href="<%=basePath%>haihang/Css/common.css" rel="stylesheet" type="text/css"> 
<link href="<%=basePath%>haihang/Css/sea.css" rel="stylesheet" type="text/css">
<link href="<%=basePath%>haihang/Css/style.css" rel="stylesheet" type="text/css">
</head>
<style type="text/css">
...
<div class="clearfix simpleHead01">
	<div class="fl">
    	<a href="{:U('Index/index')}"><img src="<%=basePath%>haihang/Images/logo.png" width="173" height="57" alt="干将网贷" /></a><span>联合登录</span>
    </div>
    <div class="fr"></div>
</div>
<!--head end-->
...
<div class="main registerAll page154">
	...
	<div class="clearfix module padding1">
        <div class="content left">
        	...
        	<p>您的QQ没有绑定任何干将网贷账户</p>
            <p>您可使用本QQ注册一个新账户，或将此QQ绑定到已有账户。</p>
			                ...
			                <li class="pad">
			                  <div class="user">
			            					 <div class="ico-p pos2"></div>
			            						...
			            						<i id="pwd_right_2" class="ico_all size15"></i><a href="{:U('Log/findPwd')}" class="blue">忘记密码?</a>
			                      </div>
...
<!--foot end-->
<script type="text/javascript" src="<%=basePath%>haihang/Scripts/jquery.form.js"></script>
<script type="text/javascript" src="<%=basePath%>haihang/Scripts/jquery.js"></script>
<script type="text/javascript" src="<%=basePath%>haihang/Scripts/appmeasurement-1.2.1-min.js"></script>
<script type="text/javascript" src="<%=basePath%>haihang/Scripts/commonquick.js"></script>
<script type="text/javascript" src="<%=basePath%>haihang/Scripts/gt.js"></script>
<script type="text/javascript" src="<%=basePath%>haihang/Scripts/register1.js"></script>
<script type="text/javascript" src="<%=basePath%>haihang/Scripts/visitorapi-1.2.1-min.js"></script>
				 ...
				 $.ajax({
				  ...
				  success:function(returnData){
						  if(returnData!="Y") {
								...
								var fwurl="<%=basePath%>qq/user/loginQQenter.action";
								$.post(fwurl,{loginname:loginname,password:password},function(data){
									if(data=="fail"){
										...
										return false;
			 							...
			 							var imgUrl="${mapUser['uImg']}";
										var nkName="${mapUser['nkName']}";
			 							window.location.href="<%=basePath%>qq/bangDingQQLogin.action?loginname="+nkName+"&uImg="+imgUrl;
									}
								});
						  }					
				  },
...
<script>
	...
	function regAccount(){
		var thirdPbLogin="${thirdPbLogin}";
		window.location.href="<%=basePath%>qq/lgIndexForQQ.action?thirdPbLogin="+thirdPbLogin;
	}
</script>
...
<!-- jQuery(function() {
		...
		function init(){
			...
			if(window.localStorage){
				 ...
				 if(!vr)return;
				 $("#uname2").val(vr);
				 setTimeout(function(){$("#pwd2").focus();},300);
  			}
		} -->
		...
		function saveCookie() {
			if ($("#saveid").attr("checked")) {
				$.cookie('loginname', $("#uname2").val(), {
					expires : 7
				});
				$.cookie('password', $("#pwd2").val(), {
					expires : 7
				});
			}
		}
