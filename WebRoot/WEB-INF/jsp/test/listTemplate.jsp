	 	 		...
	 	 		<c:forEach items="${pagehelper.list}" var="user"> 
		 	 		<tr class="text-center">
				 		...
				 		<td>${user.remark}</td>
   				 		<td>
  				 			<div class="btn-group">
								<button class="btn btn-default" onclick="userLogTest.editshow(this)" 
								data-opid="${user.id}" data-remark="${user.remark }" data-username="${user.username }" data-opercontent="${user.opercontent }">修改</button>
  							</div>
 				 		</td> 
 		 	 		</tr>
	 	 		</c:forEach>
