...
<script type="text/javascript">
	var bindcard = {
		...
		addBankCard : function() {
			window.open("${pageContext.request.contextPath}/huifu/userBindCard/tobindBankCard.action");
		},
		/* 删除银行卡 */
		delCard : function(id) {
			if (confirm("是否删除？")) {
				var url = "${pageContext.request.contextPath}/huifu/userBindCard/delBankCard.action";
				var param = {
					"id" : id
				}
				var callback = function(data) {
					...
					alert(json.RespDesc);
					window.location.href = "${pageContext.request.contextPath}/user/bankcard/queryBankCard.action";
				}
					...
					<c:forEach items="${cardList}" var="card">
						<div id="numTwo_01">
							<div id="numTwo_01_01">
								<c:choose>
									<c:when
										test="${card.isfastbindcard==1 && card.isdefaultcard == 0}">
										<button onclick="bindcard.delCard('${card.id}')">删除</button>
									</c:when>
									<c:otherwise>
										<label>默认取现卡</label>
									</c:otherwise>
								</c:choose>
								<c:if test="${card.isfastbindcard==2}">
									<a
										href="${pageContext.request.contextPath}/user/bankcard/unBindCardPage.action"
										target="_blank">解除绑定</a>
								</c:if>
							</div>
							<div id="numTwo_01_02">
								<p>${card.bankname}</p>
