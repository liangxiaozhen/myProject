				...
				<div class="fl pad_l30 pad_t30 pad_b40 pad_r30 wid_w900 clearfix">
					...
					<div
						...
						<ul class="tab_title clearfix">
							<li
								class="fl ui-select-listBox-list--now ui-select-listBox-list active"
								value="1"><a href="javascript:;" class="pad_b10">资金流动</a></li>
					...
					<form action="list.action" id="fundsLog" method="post"
						class="nwd-formUi">
									...
									<dl class="fl searchdl">
										...
										<dd class="fl a_btn wid_800">
											<span> <a href="javascript:;" id="type0"
												...
												class="fc_6 mar_l5 mar_r5">充值</a> <a
												href="javascript:void(0)" id="type2"
												class="fc_6 mar_l5 mar_r5">充值手续费</a> <a
												href="javascript:void(0)" id="type3"
												class="fc_6 mar_l5 mar_r5">提现</a> <a
												...
												<a href="javascript:void(0)" id="type17" value="17"
												class="fc_6 mar_l5 mar_r5 dp" style="display: none;">利息管理费</a>
														<!-- 
									...
									<table class="table fc_6 mar_t5 bor_t">
										<tbody>
											<tr>
												...
												<td class="fc_3">收入</td>
												<td class="fc_3">支出</td>
												<td class="fc_3">可用余额</td>
												<td class="fc_3">说明</td>
											</tr>
											<c:forEach items="${pagehelper.list }" var="item">
												<tr>
													<td class="Numfont table_timelog" style="width: 100px;"><fmt:formatDate
															pattern="yyyy-MM-dd HH:mm:ss" value="${item.recordtime }" /></td>
													<td class="fc_3"><span>${state.getUserAccountInExType(item.type)}</span></td>
													<td class="fc_3"><span style="color: red;"> <c:if
																test="${item.inamount == 0.0 }">--</c:if> <c:if
																test="${item.inamount != 0.0 }"><fmt:formatNumber pattern="##,###,##0.00" value="${item.inamount }"/></c:if>
													</span></td>
													<td class="fc_3"><span style="color: green;"> <c:if
																test="${item.outamount == 0.0}">--</c:if> <c:if
																test="${item.outamount != 0.0 }"><fmt:formatNumber pattern="##,###,##0.00" value="${item.outamount }"/></c:if>
													</span></td>
													<td class="fc_3"><fmt:formatNumber pattern="##,###,##0.00" value="${item.balance }"/></td>
													<td class="fc_3">${item.description }</td>
												</tr>
											</c:forEach>
										</tbody>
									</table>
...
$(document).ready(function(){
    ...
    */
	...
	function queryAllPerson(pageNum, pageSize) {
		location.href = "list.action?pageNum=" + pageNum + "&pageSize="
				+ pageSize+"&btime="+"${startime}"+"&etime="+"${enddate}"+"&sign="+"${sign}"+"&type="+"${type}";
	}
