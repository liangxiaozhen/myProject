...
<title>PublicloanInfoPic.jsp</title>
...
<div id="ht"><h2><span class="glyphicon glyphicon-yen"></span><em>图片区</em></h2></div>
 	<form class="form-horizontal text-center" role="form" enctype="multipart/form-data" method="post" action="${pageContext.request.contextPath}/user/common/adduserCommonpic.action">
	     <div class="container" style="margin-top: 25px;">
	         ...
	         <c:forEach items="${infoNeedpics}" var="need" varStatus="stat">
	             <!-- 引用资料编号 -->
	             <%-- <input type="hidden" name="liqno" value="${need.itemQuote.liqno}"/> --%>
	             <div class="form-group">
	             <!-- 文本选择填写 -->
	     		   <label class="col-sm-3 control-label">${need.infoname}:</label>
	     		   <div class="col-sm-3">
	     		     <!-- 引用资料内容 -->
	     		     <input type="file" name="${need.infoname},${need.itemQuote.liqno}" class="form-control"/>
	     		   </div>
	     		    <div class="col-sm-2">
