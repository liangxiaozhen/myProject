...
<script
	src="${pageContext.request.contextPath}/js/regactivity/updateActivity.js"></script>
<!-- 日历 -->
		...
		<div class="row clearfix">
			<div class="col-md-12 column">
				<form method="post" class="form-horizontal"
					id="updateRegActivityForm"
					action="${pageContext.request.contextPath}/admin/activity/editRegActAwardRule.action">
					<div class="well text-center"
						style="font-size: 14px; background: #d6e9c6;">请先完成注册活动规则设置,然后再进行注册活动奖励规则设置</div>
					<input name="id" value="${detail.id}" hidden="hidden" /> <input
						name="actid" value="${detail.id}" hidden="hidden" />
					<div class="form-group">
						<div class="col-md-10">
							<label class="col-sm-4 text-right">活动编号</label>
							<div class="col-sm-4">
								<label>${detail.actno}</label>
							</div>
						</div>
					</div>
					<div class="form-group">
						<div class="col-md-10">
							...
							<div class="col-sm-4">
								<input type="text" name="actname" class="form-control"
									value="${detail.actname}" placeholder="请输入活动名称" />
							</div>
						</div>
					</div>
					<div class="form-group">
						<div class="col-md-10">
							<label class="col-sm-4 text-right">生效日期</label>
							<div class="col-sm-3">
								<input type="text" id="actbtime" name="actbtime"
									class="Wdate form-control"
									onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',onpicked:regUpdate.check_bDate,minDate:'%y-%M-%d'})"
									class="form-control"
									value="<f:formatDate value='${detail.actbtime}' pattern='yyyy-MM-dd HH:mm:ss'/>"
					...
					<div class="form-group">
						<div class="col-md-10">
							<label class="col-sm-4 text-right">截止日期</label>
							<div class="col-sm-3">
								<input type="text" id="actetime" name="actetime"
									class="Wdate form-control"
									onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',onpicked:regUpdate.check_eDate,minDate:'%y-%M-%d'})"
									value="<f:formatDate value='${detail.actetime}' pattern='yyyy-MM-dd HH:mm:ss'/>"
					...
					<div class="form-group">
						<div class="col-md-10">
							<label class="col-sm-4 text-right">是否需要审核</label>
							<div class="col-sm-3">
								<select name="isauditalist" class="form-control">
									...
									<c:if test="${!empty isaudit_map}">
										<c:forEach items="${isaudit_map}" var="audit">
											<c:choose>
												<c:when test="${detail.isauditalist eq audit.key}">
								...
								<select name="isconsiderregtime" class="form-control">
									...
									<c:if test="${!empty isconsiderregtime_map}">
										<c:forEach items="${isconsiderregtime_map}"
											...
											<c:choose>
												<c:when
													test="${detail.isconsiderregtime eq isconsiderregtime.key}">
								...
								<select name="istemplet" class="form-control">
									...
									<c:if test="${!empty istemplet_map}">
										<c:forEach items="${istemplet_map}" var="istemplet">
											<c:choose>
												<c:when test="${detail.istemplet eq istemplet.key}">
						...
						<div class="col-md-10">
							...
							<div class="col-sm-4">
								<textarea rows="5" name="remark" class="form-control"
									placeholder="注册活动规则备注">${detail.remark}</textarea>
							</div>
						</div>
					</div>
					<div class="form-group" style="margin-top: 30px;">
						<div class="col-md-10">
							<div class="col-sm-7 text-right">
								<c:if test="${show ne 'show'}">
									<input type="submit" class="btn" value="继续编辑子活动"
										style="margin-right: 20px;" />
								</c:if>
								<c:if test="${show eq 'show'}">
									<input type="button" class="btn" value="新增子活动"
										style="margin-right: 20px;" onclick="regUpdate.insert()" />
								</c:if>
								<input type="button" class="btn" value="保存本活动"
									onclick="regUpdate.save(this)" />
							</div>
						</div>
					</div>
				</form>
