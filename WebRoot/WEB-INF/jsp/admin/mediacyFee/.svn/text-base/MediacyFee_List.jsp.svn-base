			...
			<form class="form-horizontal" id="" action="" method="post">
					...
					<div class="col-sm-8 classify">
						<p>
							<a href="javascript:void(0);" class="grade beijing">按等级</a>
							 ...
							 <a href="javascript:void(0);" class="subclass" onclick="mediacyFeeSave.low_saveUserGradeCategory(this)">新增子分类</a>
				...
				<div class="categoryBox" id="categoryBox">
 					<div class="category">
 						...
 						<div class="form-group  has-feedback hidee" style="display: block;">
							...
							<div class="col-sm-9">
								<select class="aos drawshow userGraderSel" style="border-radius: 4px;" onchange="mediacyFeeSave.low_UserGradeSelectChange(this)">
  								...
  								<span class="delete deletee cude cifen"> <a href="javascript:void(0);" onclick="mediacyFeeSave.low_delete(this)" style="color:red;">删除此分类</a></span>
						...
						<div id="system_business" class="form-group  has-feedback"
							...
							<div class="col-sm-9">
								<select class="aos abasb tendRiskSel" style="border-radius: 4px;" onchange="mediacyFeeSave.low_TendRiskSelectChange(this)" >
						...
						<div class="form-group  has-feedback">
							...
							<div class="col-sm-8 classify">
								<p>
									<span class="shuru">
										<input type="text" name="" id="" value="" class="insertRate" style="width:100px;" onkeyup="mediacyFeeSave.low_vidKeyUp(this)"/>%
									</span>
									 &nbsp; &nbsp;<span id="bn" style="color: red;"></span>
								<p>
							</div>
						...
						<div class="form-group  has-feedback">
							...
							<div class="col-sm-8 classify">
								<p>
									<span class="shuru">
										<input type="text" name="" id="" value="" class="maxFee" style="width:100px;" onkeyup="mediacyFeeSave.low_vidKeyUp(this)"/>
									</span>元
									 &nbsp; &nbsp;<span id="bn" style="color: red;"></span>
								<p>
							</div>
				...
				<div class="form-group has-success has-feedback">
					<label class="control-label col-sm-2"></label>
 					<div class="col-sm-3">
						<button type="button" id="save_edit" class="btn btn-default" onclick="mediacyFeeSave.low_save(this)" id="low_save">保存</button>
					</div>
 				</div>
			</form>
 		</div>
			...
			if(!isEmpty(remark)){
 			...
 			var list = {};
  			for(var i = 0 ;i < categoryLength ; i++){
  				...
  				MediacyFee.mfrate = $category.eq(i).find(".insertRate").val();
  				MediacyFee.maxiefee = $category.eq(i).find(".maxFee").val();
  				 list[i] =  MediacyFee;
  			}
 			...
 			$.tzAdminAjax.request({
				...
				callback:function(data){
					 ...
					 alert(obj.Msg);
					 window.location.href= basePath + "/admin/mediacyFee/list.action";
 				}
			...
			for(var i = 0 ; i < categoryLength ; i++){
				...
				var insertRateValue = $category.eq(i).find(".insertRate").val();
 				if(isEmpty(insertRateValue) || insertRateValue < -1){
					Msg = "分类"+(i+1)+"未填写标的居间费率！请填标的居间费率";
					falg = false;
					break;
				}
 				
 				if(insertRateValue == 0){
					Msg = "分类"+(i+1)+"未填写标的居间费率！请填写标的居间费率!必须大于0";
					falg = false;
					break;
				}
				...
				if(!reg.test(insertRateValue)){
					Msg = "分类"+(i+1)+"填写标的居间费率格式错误！请填写正确的标的居间费率！精确到小数点后两位";
					falg = false;
					break;
				}
				
				if(insertRateValue > 100){
					Msg = "分类"+(i+1)+"填写标的居间费率格式错误！请填写正确的标的居间费率！标的居间费率不能超过100%";
					falg = false;
					break;
				}
				...
				if(isEmpty(maxFeeValue) || maxFeeValue < -1){
					Msg = "分类"+(i+1)+"未填写最高标的居间费！请填写最高标的居间费";
					falg = false;
					break;
				}
				
				if(maxFeeValue == 0){
					Msg = "分类"+(i+1)+"最高标的居间费必须大于0";
					falg = false;
					break;
				}
				...
				if(!reg.test(maxFeeValue)){
					Msg = "分类"+(i+1)+"填写最高标的居间费格式错误！请填写正确的最高标的居间费！精确到小数点后两位";
					falg = false;
					break;
				}
 			}
...
" 					</div>"
		...
		low_UserGradeSelectChange:function(obj){
			...
			if(userGraValue == "10000"){//选择全部会员
				...
				"<button type='button' class='btn btn-default btn-none' value='10000' style='margin-right: 15px; float: left' onclick='mediacyFeeSave.low_UserGradeDelet(this)'>"+
				"全部会员等级</button>";
		...
		low_UserGradeObjectShow:function(UserGra,vaule,name){
			if(UserGra == "UserGra"){
 				var html = "<button type='button' class='btn btn-default btn-none' value='"+vaule+"' style='margin-right: 15px; float: left' onclick='mediacyFeeSave.low_UserGradeDelet(this)'>"+
				""+name+""+
				"<span style='color: red' class='glyphicon glyphicon-remove'></span>"+
				"</button>";
				return html;
			}else{
				var html = "<button type='button' class='btn btn-default btn-nonee' value='"+vaule+"' style='margin-right: 15px; float: left' onclick='mediacyFeeSave.low_UserGradeDelet(this)'>"+
				""+name+""+
