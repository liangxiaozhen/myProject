...
<head>
    ...
    <script type="text/javascript">
        ...
        //上架前判断 单标前端信息未填数量
        function putawayBefore(id) {
            $.post("${pageContext.request.contextPath}/admin/tenderItem/putawayBefore.action", {id: id},
                function (data) {
                    var otd = eval('(' + data + ')');
                    if (otd.result == "tstatus_4") {
                        alert("提示:已流标的标不能再上架");
                    }else if(otd.result>0){
                        var flag=confirm("提示：\n        该标的单标前端信息还有--"+otd.result+"--个项目的内容未填写，你确定要上架吗?");
                        if(flag){
                            shelves(id);
                        }
                    }else if(otd.result == 0){
    ...
    <div class="row clearfix">
        <div class="col-md-12 column">
            ...
            <form id="selecttenderItemByCondition" method="post"
                  action="${pageContext.request.contextPath}/admin/tenderItem/selectTenderItemByCondition.action">
            ...
            <table class="table  table-hover" id="personList_table">
                ...
                <c:forEach items="${pagehelper.list }" var="tenderItem" varStatus="tendsta">
                    <tr class="text-center">
                        ...
                        <td>
                            <c:choose>
                                <c:when test="${tenderItem.tstatus==2&&tenderItem.onanddown==0}">
                                    <span style="color: blue">待上架</span>&nbsp;&nbsp;<span><a style="cursor: pointer;color:black "
                                    onclick="putawayBefore('${tenderItem.id}')">上架 </a></span>
                                </c:when>

                                <c:when test="${tenderItem.onanddown==1}">
                                    <span style="color: blue">上架</span>&nbsp;&nbsp;<span><a style="cursor:pointer;color: black" onclick="theshelves('${tenderItem.id}')">下架</a></span>
                                </c:when>

                                <c:when test="${tenderItem.onanddown==2}">
                                    <span><a style="cursor: pointer;color:black " onclick="putawayBefore('${tenderItem.id}')">上架 </a> </span>&nbsp;&nbsp;<span style="color: red">下架</span>
                                </c:when>
                        ...
                        <td>
                            <c:choose>
                                <c:when test="${tenderItem.tstatus == 1||tenderItem.tstatus == 9}">
                                    <button class="btn btn-default" onclick="toUpdateUi('${tenderItem.id}')">修改</button>
                                </c:when>
                                <%--投标资金是否在冻结中（0否，1是），投标状态为（1.待审核 4.已完成 5.处理中 6.处理失败）都视为冻结中，就不显示手动流标这个按钮--%>
                                <c:when test="${(tenderItem.tstatus == 3||tenderItem.tstatus==5)&&tenderItem.isfroze==0}">
                                    <button class="btn btn-default" onclick="flowstandard('${tenderItem.id}')">手动流标
                                    </button>
                                </c:when>

                                <c:when test="${tenderItem.tstatus == 5}">
                                    <button class="btn btn-default" onclick="loan('${tenderItem.id}')">放款</button>
                                </c:when>

                                <c:when test="${tenderItem.tstatus == 6}">
                                    <button class="btn btn-default" onclick="createplan('${tenderItem.id}')">生成还款计划
                                    </button>
                                </c:when>
