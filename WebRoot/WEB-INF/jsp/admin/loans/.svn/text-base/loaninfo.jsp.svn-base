		...
		<div class="row clearfix">
			<div class="col-md-12 column"
				...
				<form id="selectgfundsIntByCondition" method="post"
					action="${pageContext.request.contextPath}/loan/selectIdAndauditstatus.action">
					...
					<div>
						<button id="query_btn" class="btn btn-default"
							onclick="selectgfundsIntByCondition(1,9)">查询</button>
					...
					<tbody>
						<c:forEach items="${pagehelper.list }" var="gfundsInt">
							<tr>
								...
								<td><a
									href="${pageContext.request.contextPath}/picpath/pictureadmin.action?liano=${gfundsInt.liano}">查看图片</a></td>
								<td>${gfundsInt.auditman}</td>
								<td><fmt:formatDate value="${gfundsInt.audittime}"
										type="date" pattern="yyyy-MM-dd" /></td>
								<td>
									<button class="btn"
										onclick="toUpdateUi('${gfundsInt.id}','1','${adminuser.username }')">通过</button>
									<button class="btn" id="modify"
										onclick="toUpdateUi('${gfundsInt.id}','2','${adminuser.username}')">拒绝</button>
								</td>
							</tr>
						</c:forEach>
					</tbody>
