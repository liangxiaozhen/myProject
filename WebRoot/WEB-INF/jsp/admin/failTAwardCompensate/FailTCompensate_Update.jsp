...
<title>failTAwardCompensate_updateUI</title>
...
<script src="${pageContext.request.contextPath}/js/failTAwardCompensate/updatefailTAwardCompensate.js"></script>
...
$(function(){
	...
	function gotofailTAwardCompensateList(){
		   window.location.href="${pageContext.request.contextPath }/admin/failTAwardCompensate/getfailTAwardCompensateByCondition.action";
	}
</script>
<style type="text/css">
...
<div class="container" style="margin-top: 25px;">
		<form class="form-horizontal" role="form" id="defaultForm" action="${pageContext.request.contextPath}/admin/failTAwardCompensate/updatefailTAwardCompensate.action">
			<c:if test="${empty failTAwardCompensate}">
			     <label>暂无数据</label>
			</c:if>
			<c:if test="${!empty failTAwardCompensate}">
			   <c:forEach items="${failTAwardCompensates}" var="fail" varStatus="sta">
				...
				<!--投标累计金额分段最低与最高-->
				<input type="hidden" name="failTAwardCompensates[${sta.index}].id" value="${fail.id}"/>
					<div class="form-group">
						<label for="inputminMoney" class="col-sm-3 control-label">投标累计金额最低-最高</label>
						<div class="col-sm-3">
							<div class="input-group">
								<input type="text" name="failTAwardCompensates[${sta.index}].minmoneyaward" id="inputminMoney"
									 value="${fail.minmoneyaward}" class="form-control" /> <span
									class="input-group-addon">元</span>
							</div>
						</div>
						<div class="col-sm-3">
							<div class="input-group">
								<input type="text" name="failTAwardCompensates[${sta.index}].maxmoneyaward" id="inputmaxMoney"
									 value="${fail.maxmoneyaward}" class="form-control" /> <span
									class="input-group-addon">元</span>
							</div>
						</div>
					</div>
					...
					<div class="form-group" id="quotainput_div">
						...
						<div class="col-sm-3">
							<div class="input-group">
								<input type="text" name="failTAwardCompensates[${sta.index}].quota" id="inputquota"
					...
					<div class="form-group" id="dayawardrate_div">
						...
						<div class="col-sm-3">
							<div class="input-group">
								<input type="text" name="failTAwardCompensates[${sta.index}].dayawardrate" id="dayAwardRate"
						...
						<div class="col-sm-3">
							<div class="input-group">
								<input type="text" name="failTAwardCompensates[${sta.index}].maxcompensate" id="maxCompensate"
			...
			<div class="form-group">
				...
				<div class="col-sm-3">
					...
					<c:if test="${failTAwardCompensate.isaudit eq 1}">
					    <option value="1">是</option>
					</c:if>
					<c:if test="${failTAwardCompensate.isaudit eq 0}">
			...
			<div class="form-group">
				...
				<div class="col-sm-3">
					...
					<c:if test="${failTAwardCompensate.istemplet eq 1}">
					    <option value="1">是</option>
					</c:if>
					<c:if test="${failTAwardCompensate.istemplet eq 0}">
					    <option value="0">否</option>
					</c:if>
					</select>
				</div>
			...
			<div class="form-group">
			   ...
			   <div class="col-sm-3">
			     <textarea rows="3" name="remark" class="form-control">${failTAwardCompensate.remark}</textarea>
			   </div>
			</div>
			<div class="form-group">
...
<%-- <div id="id"></div>
<form action="${pageContext.request.contextPath}/failTAwardCompensate/updatefailTAwardCompensate.action" method="post" id="updatefailTAwardCompensateForm">
<input type="hidden" value="${failTAwardCompensate.id }" name="id">
					...
					<div class="row" style="line-height: 0px;" id="minmoney-maxmoney">
							<div class="col-md-10   col-md-offset-1"  >
										 <font size="3">分段最低及最高金额：</font>&nbsp;&nbsp;<input type="text" name="minmoney"  value="${failTAwardCompensate.minmoney }"  style="width:100px;height:25px;font-size: 15px"/> 元-<input type="text" name="maxmoney"   value="${failTAwardCompensate.maxmoney }"  style="width:100px;height:25px;font-size: 15px"/> 元
							 </div>
					</div>	
					<hr>
					<div class="row" style="line-height: 0px;">
						<div class="col-md-3  col-md-offset-1">
							<font size="3">利息方式补偿开关</font>&nbsp;&nbsp;：
								<select name="isintcompensateon"  id="isintcompensateon" style="width:100px;height:25px;font-size: 15px">
									...
									<option value="1" <c:if test="${failTAwardCompensate.isintcompensateon==1 }">selected="selected"</c:if> >开</option>								
									<option value="0" <c:if test="${failTAwardCompensate.isintcompensateon==0 }">selected="selected"</c:if> >关</option>							
								</select>
						</div>
						<div class="col-md-4  ">
							<font size="3">奖品补偿方式开关</font>&nbsp;&nbsp;：
								<select name="isawardcompensateon"  id="isawardcompensateon" style="width:100px;height:25px;font-size: 15px">
									<option value="">请选择</option>
									<option value="1" <c:if test="${failTAwardCompensate.isawardcompensateon==1 }">selected="selected"</c:if> >开</option>
									<option value="0" <c:if test="${failTAwardCompensate.isawardcompensateon==0 }">selected="selected"</c:if> >关</option>
								</select>
						</div>
					</div>
					<hr>
							...
							<div class="row">		
									<div class="col-md-2 col-md-offset-1" >
													...
													<select id="type"  name="type" style="width:80px;height:25px;font-size: 15px" >
														<option value="">请选择</option>
														<option  value="iequota" <c:if test="${failTAwardCompensate.quota!=null }">selected="selected"</c:if> >定额</option>
														<option value="iepercent" <c:if test="${failTAwardCompensate.dayawardrate!=null }">selected="selected"</c:if> >百份比</option>
													</select>
												</div>
												<script type="text/javascript">
											...
											<div  id="iequota" class="col-md-4">
													<font size="3">定额补偿金</font>&nbsp;&nbsp;：<input type="text" name="quota"   value="${failTAwardCompensate.quota }"  style="width:100px;height:25px;font-size: 15px"/>元
											</div>
											<div id="iepercent" >
													<div  class="col-md-3">
															<font size="3">日奖励费率</font>&nbsp;&nbsp;：<input type="text" name="dayawardrate"    value="${failTAwardCompensate.dayawardrate }"  style="width:100px;height:25px;font-size: 15px"/> 
													</div>
													<div  class="col-md-5">
															<font size="3">最高补偿金额：</font>&nbsp;&nbsp;<input type="text" name="maxcompensate"  value="${failTAwardCompensate.maxcompensate }"  style="width:100px;height:25px;font-size: 15px"/> 
													</div>
											</div>
							</div>
				<hr>
				...
				<div id="awardcompensateon">
						...
						<div class="col-md-4 col-md-offset-1">
							<font size="3">是否设为模板</font>&nbsp;&nbsp;：
								<select name="istemplet" style="width:200px;height:25px;font-size: 15px">
										<option value="">请选择</option>
										<option value="1" <c:if test="${failTAwardCompensate.istemplet==1 }">selected="selected"</c:if> >是</option>
										<option value="0" <c:if test="${failTAwardCompensate.istemplet==0 }">selected="selected"</c:if> >否</option>
								</select>
						</div>
					</div>
					<hr>	
					<div class="row" style="line-height: 0px;">
						<div class="col-md-5">
							...
							<button id="backid" class="btn btn-success" name="backid" type="button" onclick="gotofailTAwardCompensateList()">返回列表</button>
						</div>
					</div>
				</form> --%>
</body>
