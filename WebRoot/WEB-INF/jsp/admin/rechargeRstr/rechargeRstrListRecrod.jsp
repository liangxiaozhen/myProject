	...
	<div id="itemList_table">
		...
		<div style="float: left; margin-left: 25px; width: 90%">
			<form id="form-select" method="post"
				action="${pageContext.request.contextPath}/admin/rechargeRstr/queryAllRecrod.action">
						...
						<tr id="item_tr_${item.id }" class="text-center2">
							...
							<td>
								<p class="remark-p text-center" data-toggle="tooltip"
									title="${item.ugradeStr}">${item.ugradeStr}</p>
							</td>
							...
							<td><c:if test="${item.proxypayman eq '0'}">-----</c:if> <c:if
									test="${item.proxypayman eq 'MDT000001'}">莫邪软件</c:if> <c:if
									test="${item.proxypayman eq 'SDT000001'}">担保账户1</c:if> <c:if
									test="${item.proxypayman eq 'SDT000002'}">风险保证金账户1</c:if> <c:if
									test="${item.proxypayman eq 'BASEDT'}">BASEDT</c:if></td>
							...
							<td><c:if test="${item.isuse eq 1}">
									<p data-toggle="tooltip" style="color: green">${item.isuseStr }</p>
								</c:if> <c:if test="${item.isuse eq 0}">
									<p data-toggle="tooltip">${item.isuseStr }</p>
								</c:if></td>
