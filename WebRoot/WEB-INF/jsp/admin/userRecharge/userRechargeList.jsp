...
function requestQuery(id){
	var  action = "admin/userRecharge/requestQuery.action";
	var params = {
			"id" : id
		};
		$.post(action, params, requestQuery_success_callback);
}
/*
 *详情 的回调函数 returnData标识contorller返回的jsonStr字符串
 */
...
//接收数据包
function respsneResult(id){
	var  action = "admin/userRecharge/respsneResult.action";
	var params = {
			"id" : id
		};
		$.post(action, params, respsneResult_success_callback);
}
/*
 *详情 的回调函数 returnData标识contorller返回的jsonStr字符串
 */
		...
		<div class="row clearfix">
			<div class="col-md-12 column">
				<div style="float: left;">
					<form method="post"
						action="${pageContext.request.contextPath}/admin/userRecharge/query.action"
						id="form-select">
						<div style="margin-top: 8px">
							...
							<label>充值订单号:&nbsp;</label> <input type="text"
								id="rechargeno_select" name="rechargeno" style="width: 180px"
								placeholder="充值订单号" /> <label>银行返回充值订单号:</label> <input
				...
				<table class="table table-hover" id="userRechargeList_table">
					...
					<tbody>
						...
						<c:forEach items="${pagehelper.list}" var="userRecharge"
							...
							<tr id="userRecharge_tr_${userRecharge.id }" class="text-center2">
								<td>${status.count}</td>
								...
								<td>
									<c:if test="${!empty userRecharge.recharfee}">
										${userRecharge.recharfee}
									</c:if>
									<c:if test="${empty userRecharge.recharfee}">
										0.00
									</c:if>
								</td>
								<td><c:if test="${!empty userRecharge.starttime}">${sf.format(userRecharge.starttime)}</c:if></td>

								<td>${userRecharge.ubai.realname}</td>
								<td>
									<c:if test="${userRecharge.rechargetype eq 1}">个人网银</c:if>
