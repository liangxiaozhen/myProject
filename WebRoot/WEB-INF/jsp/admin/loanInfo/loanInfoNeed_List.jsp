...
<head>
    ...
    <script type="text/javascript">
        ...
        function addloan() {
            var url = "${pageContext.request.contextPath}/admin/loanInfo/loanInfoNeed_ui.action";
            window.location.href = url;
        }
        //详情
        function toDetail(id) {
            var action = "${pageContext.request.contextPath}/admin/loanInfo/toDetailNeed.action";
            var param = {
                "id": id
            }
        ...
        function toDeletByiscite() {
            var id = $("#modelID").val();
            var action = "${pageContext.request.contextPath}/admin/loanInfo/toDeleteNeed.action";
            var param = {
                "id": id
            }
            var callback = function (data) {
        ...
        //编辑
        function update(id) {
            var url = "${pageContext.request.contextPath}/admin/loanInfo/updateloanInfoNeed.action?id=" + id;
            window.location.href = url;
        }

        //停用和启用
        function Toenableneed(id, str) {
            var action = "${pageContext.request.contextPath}/admin/loanInfo/toenableneed.action";
            var param = {
                "id": id,
                "str": str
            }
            ...
            <tbody>
                <c:forEach items="${pagehelper.list}" var="loan" varStatus="stas">
                    <tr class="text-center">
                        ...
                        <td>${loan.infoname}</td>
                        <td>
                            <c:if test="${loan.infotype eq 1}">
                                上传图片
                            </c:if>
                            <c:if test="${loan.infotype eq 2}">
                                文本
                            </c:if>
                        </td>
                        <td>
                        ...
                        <td>
                            <c:if test="${loan.isneed eq 1}">
                                <span style="color: blue;">启用</span>&nbsp;&nbsp;<span><a style="cursor:pointer;color:black" onclick="Toenableneed('${loan.id}','t')">停用</a></span>
                            </c:if>
                            <c:if test="${loan.isneed eq 0}">
                                <span><a style="cursor:pointer;color: black;" onclick="Toenableneed('${loan.id}','q')">启用</a></span>&nbsp;&nbsp;<span style="color:red">停用</span>
                            </c:if>
                        </td>
                        <td>
                            <button class="btn btn-default" id="update" onclick="update('${loan.id}')">编辑</button>
                            <button class="btn btn-default" id="toDelete"
                                    onclick="todelete('${loan.id}','${stas.count}','${loan.isneed}')">删除
                            </button>
                        </td>
                        <td>
                            <button class="btn btn-default" id="Detail" onclick="toDetail('${loan.id}')">详情</button>
                        </td>
                    </tr>
                </c:forEach>
            </tbody>
