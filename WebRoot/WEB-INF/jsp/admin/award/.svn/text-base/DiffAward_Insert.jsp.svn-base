...
<body style="label-family:'微软雅黑'; ">
	<form id="insertDiffAwardForm" class="form-horizontal" action="${pageContext.request.contextPath}/admin/diffAwardSwitch/insertDiffAward.action" method="post">
		<div class="container" style="margin-top: 80px;" >
			<!--奖品属性名称-->
			<div class="form-group">
				<label for="isdebtaudit" class="col-sm-3 control-label laber_from">奖品属性名称：</label>
				<div class="col-sm-3">
					<select id="attribute" name="attribute" class="form-control">
              			 <option value="">请选择</option>
              			 <c:forEach items="${attributeData}" var="ad">
			...
			<!-- <div class="form-group">
				...
				<div class="col-sm-3">
					<input type="text" name="operateTime" id="operateTime" class="Wdate form-control" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
				</div>
			</div> -->
...
<script type="text/javascript">
	...
	$().ready(function(){
		...
		jQuery.validator.addMethod("attributeIsOnly",function(value,element){
			
			var attribute = $('select[name="attribute"]').find("option:selected").val();
			//alert("attribute====="+attribute);
			var flag = true;
			$.ajax({
				url:"<%=basePath%>admin/diffAwardSwitch/attributeIsOnly.action",
				dateType:"json",
				data:{
					attribute:attribute
				},
		...
		$("#insertDiffAwardForm").validate({
			
			rules:{
				attribute:{
					"required":true,
					attributeIsOnly:true
				},
				allSwitch:"required"
