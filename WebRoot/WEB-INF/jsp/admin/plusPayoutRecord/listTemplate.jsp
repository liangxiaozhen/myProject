		...
		<c:forEach items="${pagehelper.list}" var="user">
			<tr class="text-center">
 				...
 				<c:choose>
						...
						<c:otherwise>
 				 				<input type="checkbox" class="checkAll" value="${user.id}" />
						</c:otherwise>
					</c:choose>
 				</td>
				<td>${user.id}</td>
				...
				<td>
					<div class="btn-group">
						<button class="btn btn-default"
							onclick="plusPayoutRecord.low_detail(this)"
							data-opid="${user.id}">详情</button>
						<c:if test="${user.isaudit == 0}">
							<button class="btn btn-default"
								onclick="plusPayoutRecord.low_operation(this)"
								data-opid="${user.id}" data-pporderno="${user.pporderno}"
								data-intprofit="${user.intprofit}" data-remark="${user.remark}"
								data-rorderno="${user.rorderno}"
								data-likevoucherprofit="${user.likevoucherprofit}">审核</button>
						</c:if>
					</div>
				</td>
			</tr>
