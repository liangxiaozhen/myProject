			...
			<form class="form-horizontal" id="" action="" method="post">
					...
					<div class="col-sm-8 classify">
						<p>
							<a href="javascript:void(0);" class="grade beijing">按等级</a>
							 ...
							 <a href="javascript:void(0);" class="subclass" onclick="guaranteeFeeSave.low_saveUserGradeCategory(this)">新增子分类</a>
				...
				<div class="categoryBox" id="categoryBox">
 					<div class="category">
 						...
 						<div class="form-group  has-feedback hidee" style="display: block;">
							...
							<div class="col-sm-9">
								<select class="aos drawshow userGraderSel" style="border-radius: 4px;" onchange="guaranteeFeeSave.low_UserGradeSelectChange(this)">
  								...
  								<span class="delete deletee cude cifen"> <a href="javascript:void(0);" onclick="guaranteeFeeSave.low_delete(this)" style="color:red;">删除此分类</a></span>
						...
						<div id="system_business" class="form-group  has-feedback"
							...
							<div class="col-sm-9">
								<select class="aos abasb tendRiskSel" style="border-radius: 4px;" onchange="guaranteeFeeSave.low_TendRiskSelectChange(this)" >
						...
						<div class="form-group  has-feedback">
							...
							<div class="col-sm-8 classify">
								<p>
									<span class="shuru">
										<input type="text" name="" id="" value="" class="insertRate" style="width:100px;" onkeyup="guaranteeFeeSave.low_vidKeyUp(this)"/>%
									</span>
									 &nbsp; &nbsp;<span id="bn" style="color: red;"></span>
								<p>
							</div>
						...
						<div class="form-group  has-feedback">
							...
							<div class="col-sm-8 classify">
								<p>
									<span class="shuru">
										<input type="text" name="" id="" value="" class="maxFee" style="width:100px;" onkeyup="guaranteeFeeSave.low_vidKeyUp(this)"/>
									</span>元
									 &nbsp; &nbsp;<span id="bn" style="color: red;"></span>
								<p>
							</div>
				...
				<div class="form-group has-success has-feedback">
					<label class="control-label col-sm-2"></label>
 					<div class="col-sm-3">
						<button type="button" id="save_edit" class="btn btn-default" onclick="guaranteeFeeSave.low_save(this)" id="low_save">保存</button>
					</div>
 				</div>
			</form>
 		</div>
			...
			if(!isEmpty(remark)){
 			...
 			var list = {};
  			for(var i = 0 ;i < categoryLength ; i++){
 				 ...
 				 guaranteeFee.iepercent = $category.eq(i).find(".insertRate").val();
 				 guaranteeFee.maxiefee = $category.eq(i).find(".maxFee").val();
  				 list[i] =  guaranteeFee;
  			}
 			...
 			$.tzAdminAjax.request({
				...
				callback:function(data){
					 ...
					 alert(obj.Msg);
					 window.location.href= basePath + "/admin/guaranteeFee/edit.action";
 				}
		...
		low_saveValidation:function(obj){
			...
			for(var i = 0 ; i < categoryLength ; i++){
				...
				if(!reg.test(insertRateValue)){
					Msg = "分类"+(i+1)+"填写担保费率格式错误！请填写正确的担保费率！精确到小数点后两位";
					falg = false;
					break;
				}
				
				if(insertRateValue > 100){
					Msg = "分类"+(i+1)+"填写担保费率格式错误！请填写正确的担保费率！担保费率不能超过100%";
					falg = false;
					break;
				}
				...
				if(!reg.test(maxFeeValue)){
					Msg = "分类"+(i+1)+"填写最高担保费格式错误！请填写正确的最高担保费！精确到小数点后两位";
					falg = false;
					break;
				}
 			}
...
" 					</div>"
		...
		low_UserGradeSelectChange:function(obj){
			...
			if(userGraValue == "10000"){//选择全部会员
				...
				"<button type='button' class='btn btn-default btn-none' value='10000' style='margin-right: 15px; float: left' onclick='guaranteeFeeSave.low_UserGradeDelet(this)'>"+
				"全部会员等级</button>";
		...
		low_UserGradeObjectShow:function(UserGra,vaule,name){
			if(UserGra == "UserGra"){
 				var html = "<button type='button' class='btn btn-default btn-none' value='"+vaule+"' style='margin-right: 15px; float: left' onclick='guaranteeFeeSave.low_UserGradeDelet(this)'>"+
				""+name+""+
				"<span style='color: red' class='glyphicon glyphicon-remove'></span>"+
				"</button>";
				return html;
			}else{
				var html = "<button type='button' class='btn btn-default btn-nonee' value='"+vaule+"' style='margin-right: 15px; float: left' onclick='guaranteeFeeSave.low_UserGradeDelet(this)'>"+
				""+name+""+
