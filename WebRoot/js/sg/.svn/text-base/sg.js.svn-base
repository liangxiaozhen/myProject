...
$.tzIframe = function(options){
	var opts = $.extend({},$.tzDialog.methods,$.tzDialog.defaults,options);
	var $dialog = opts.init(opts);
	$dialog.find(".tzdialog_message").html("<div  class='tm-dialog-loading tmWindow_loading' style='position:absolute;top:30%;left:43%;'><img src="+opts.images+"><label style='font-size:12px;'>请稍后，正在加载中...</label></div>");
...
$.tzDialog.defaults = {
	...
	ghost:true,
	value:"",
	images:basePath+"/js/sg/images/loading.gif",
	animate:"top",
	cancelText:"取消",
	sureText:"确定",
...
$.tzAjax = {
	...
	ajaxMain:function(opts,_url,dataJson){
		$.ajax({
			...
			error:function(){loading("抱歉！因为操作不能够及时响应，请稍后在试...",2);opts.error();clearTimeout(ajaxTimeout);},
...
$.tzAdminAjax = {
	request : function(options,dataJson){
		...
		if(opts.limit){
			clearTimeout(ajaxAdminTimeout);
			ajaxAdminTimeout = setTimeout(function(){
				$.tzAdminAjax.ajaxMain(opts,_url,dataJson);
			},200);
		}else{
			$.tzAdminAjax.ajaxMain(opts,_url,dataJson);//做权限的时候，统一ajax拦截登陆的时候，有意义
		}
	},
	ajaxMain:function(opts,_url,dataJson){
		$.ajax({
			...
			url : _url,
			beforeSend:function(){opts.before();},
			error:function(){loading("抱歉！因为操作不能够及时响应，请稍后在试...",2);opts.error();clearTimeout(ajaxAdminTimeout);},
			success:function(data){
				if(data=="logout"){
					alert("session超时,请登录...");
