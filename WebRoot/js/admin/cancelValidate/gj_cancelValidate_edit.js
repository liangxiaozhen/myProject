var cancelValidateEdit = {
		low_findAll:function(obj){
			$(obj).text("查询中...");
			$("#search_username").val("");
			$("#box").load(basePath+"/admin/cancelValidate/insert.action");
		},
		...
		low_insert:function(obj){
			...
			var email = $(obj).data("email");
 			...
 			$("#email").val(email);
 			$("#cancelValidate_model").modal('show');
  		},
  		low_save:function(obj){
  			$("#cancelValidate_model").modal('hide');
  		    var params =$("#cancelValidate_edit_form").serialize();
   		    $.ajax({
  		    	type:"post",
  		    	url:basePath+"/admin/cancelValidate/save.action",
  		    	data:params,
   		    	success:function(data){
  		    		...
  		    		if(data == "logout"){
  		    			...
  		    			alert("保存成功！");
  		    			window.location.href = basePath+"/admin/cancelValidate/insert.action";
 	...
 	function queryAllPerson(pageNo,pageSize){
 		...
 		var pageSize = "20";
  		$.ajax({
			...
			url:basePath+"/admin/cancelValidate/userTemplate.action",
			success:function(data){
