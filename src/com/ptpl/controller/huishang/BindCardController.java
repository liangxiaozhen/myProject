 ...
 * @Description: TODO(徽商银行电子账户签约卡绑定)
 * @author zhenglm
 * @date 2017年4月1日 上午10:28:33
 *
 */
@RequestMapping("/huishang/bindCard")
@Controller
public class BindCardController extends BaseController{
	...
	@RequestMapping(value = "/saveBindingBankCard",method = {RequestMethod.POST,RequestMethod.GET})
	public void saveBindingBankCard(String mbbCardNo){
		String msg = "";
		// 从session中获取当前登录用户基本信息
		UserBaseAccountInfo userBaseAccountInfo = getUserBaseAccountInfo();
 		if(userBaseAccountInfo != null  && StringUtil.isNotEmpty(mbbCardNo)){
			boolean falg = StringUtil.checkBankCard(mbbCardNo);
			falg = true;
			if(falg){
				UserBankCard userBankCard = userBankCardService.selectBoundCardByBaseId(userBaseAccountInfo.getId());
				if(userBankCard != null){
					msg = "0"; // 保存失败
				}else{
					String code = bindCardPost(request, response, mbbCardNo);//提交绑定
	 ...
	 * 徽商银行电子账户签约卡绑定接口调用
	 * @Title: bindCardPost
	 * @Description: TODO(徽商银行电子账户签约卡绑定接口调用)
	 * @param request
	 * @param response
	 * @return void    返回类型
	 */
	private String bindCardPost(HttpServletRequest request,HttpServletResponse response,String mbbCardNo) {
		// 从session中获取当前登录用户基本信息
		UserBaseAccountInfo userBaseAccountInfo = getUserBaseAccountInfo();
		if(userBaseAccountInfo != null){
			...
			userFsAccountInfo = getDecryptionUserFsAccountInfoDetail(userFsAccountInfo);//数据库加密信息解密
			LinkedHashMap<String, String> reqMap = new LinkedHashMap<String, String>();
			// 请求报文头
			reqMap.put("TRXCODE", "5812"); // 交易代码TRXCODE
			...
			reqMap.put("HEADRESERVED", ""); // 报文头保留域HEADRESERVED——可选
			// 请求报文
	        ...
	        reqMap.put("SIG_CARD", mbbCardNo); // 绑定卡号SIG_CARD
	        reqMap.put("SURNAME", userBaseAccountInfo.getRealname()); // 姓名SURNAME
	        reqMap.put("KEYTYPE", "01"); // 证件类型KEYTYPE
	        reqMap.put("IDNO", userBaseAccountInfo.getCertificationnumber()); // 证件号码IDNO
	        reqMap.put("MOBILE", userFsAccountInfo.getFsmobile()); // 手机号码MOBILE
			reqMap.put("REMARK","绑定卡"); // 备注
			String result = null;
			try {
			...
			if(StringUtil.isNotEmpty(result)){
 				return callBack(result, userBaseAccountInfo.getId());
			}
		}
		return null;
	}
	 ...
	 * @return void    返回类型
	 */
	private String callBack(String respoResult, BigDecimal baseid){
		...
		String  TXNDATE 		= jsonObject.getString("TXNDATE"); // 签约日期
		String  TXNTIME 		= jsonObject.getString("TXNTIME"); // 签约时间
		...
		if ("00000000".equals(jsonObject.getString("RETCODE"))){
			...
			QueryCardInformationController.doParams(request, response, baseid, CARDNBR);
		} else {
